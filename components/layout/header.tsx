        ...
        <div className="ml-auto flex shrink-0 items-center gap-3 md:ml-0 md:gap-4">
          {isLoggedIn ? (
            <>
              <Link
                href="/posts/new"
                className="hidden text-sm font-semibold text-[var(--muted-ink)] transition hover:text-[var(--ink)] sm:inline"
              >
                撰寫
              </Link>
              {isAdmin ? (
                <Link
                  href="/admin"
                  className="hidden text-sm font-semibold text-[var(--brand)] transition hover:text-[var(--brand-strong)] sm:inline"
                >
                  審查後台
                </Link>
              ) : null}
